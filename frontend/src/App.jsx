...
const PROTOCOL = API.startsWith('http') ? '' : window.location.protocol + "//"
const BASE_URL = API.startsWith('http') ? API : `${PROTOCOL}${API}`
// Remove protocol for WebSocket hook as it handles it
const WS_URL = API.replace(/^https?:\/\//, '') + "/ws"
...
export default function App() {
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/api/trades`).then(r => r.json()).then(setTrades)
    fetch(`${BASE_URL}/api/wallets`).then(r => r.json()).then(setWallets)
    fetch(`${BASE_URL}/api/pnl`).then(r => r.json()).then(d => setPnl(d.total_pnl_usd))
  }, [BASE_URL])
